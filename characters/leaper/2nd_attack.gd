...
func run(delta: float) -> void:
	...
	actor.velocity.x = lerp(actor.velocity.x, actor.facing * 60.0, delta)
	actor.move_and_slide(actor.velocity, Vector2(0.0, -1.0))
	

	if actor.anims.current_animation_position > 0.2 and actor.anims.current_animation_position <= 0.3 and not hit_connected:
		var fella = actor.attack_range.get_attacked_fella()
		if fella != null:
			Console.count("2ndhit")
			hit_connected = true
